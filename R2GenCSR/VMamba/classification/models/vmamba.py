...
class VSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor,global_features=False,featuremap_folder = None):
        ####featuremap_folder
        if featuremap_folder is not None:
        ...
        for layer in self.layers:
            ...
            if featuremap_folder is not None:
                ...
                # if layer_name==0:
                #     folder_name= f'{featuremap_folder}/patch_emb'
                # else:
                #     folder_name= f'{featuremap_folder}/layer_name{str(layer_name)}'
                # folder_name= f'{featuremap_folder}/layer_name{str(layer_name)}'
            ...
            # x = layer(x)
        # feature_map= feature_map[999, :, :, channel]
        #####
        if global_features == True:
            x = self.classifier(x)
        return x

...
class FieldParser:
    ...
    def parse(self, features):
        ...
        # #初步报告
        Draft_text = features.get("Draft_text", "")
        Draft_text = self.clean_report(Draft_text)
        to_return['Draft_text'] = Draft_text
        # chest x-ray images
        images = []
        ...
        # 新加 加载node_features和adj_matrix
        # node_features = []
        # adj_matrix = []
        # # 安全加载节点特征
        # node_features = torch.load(self.args.node_features, weights_only=True)
        # adj_matrix = torch.load(self.args.adj_matrix, weights_only=True)
        # to_return["node_features"] = node_features     # 将它们添加到返回的字典中
        # to_return["adj_matrix"] = adj_matrix     # 将它们添加到返回的字典中
        # to_return["edge_index"] = edge_index
        # to_return["edge_type"] = edge_type
        return to_return
       # 新加

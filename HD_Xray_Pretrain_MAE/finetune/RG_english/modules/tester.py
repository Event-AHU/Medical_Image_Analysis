    ...
    def plot(self):
        ...
        with torch.no_grad():
            for batch_idx, (images_id, images, reports_ids, reports_masks) in tqdm(enumerate(self.test_dataloader)):
                ...
                image = torch.clamp((images[0][0].cpu() * std + mean) * 255, 0, 255).int().cpu().numpy()  #iu-xray
                report = self.model.tokenizer.decode_batch(output.cpu().numpy())[0].split()
                
                ground_truth = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())[0].split()
                ...
                for layer_idx, attns in enumerate(attention_weights):
                    ...
                    for word_idx, (attn, word) in enumerate(zip(attns, report)):
                        ...
                        cv2.imwrite(os.path.join(self.save_dir, "attentions", "{:04d}".format(batch_idx),
                                    ...
                                    heatmap)
                        with open(os.path.join(self.save_dir, "attentions", "{:04d}".format(batch_idx),
                                                    "layer_{}".format(layer_idx), 'predict.txt'), mode='w', encoding='utf-8') as f:
                            f.write(f"预测值：{' '.join(report)} \n真实值：{' '.join(ground_truth)}\n\n")
                        
                        # print(image.shape)
                        Image.fromarray(np.uint8(image.transpose(1, 2, 0))).save(os.path.join(self.save_dir, "attentions", "{:04d}".format(batch_idx),
                                                    "layer_{}".format(layer_idx),f"orgin_{batch_idx}_iu.png"))
                    
                        word_vectors =self.model(images, reports_ids, mode='train')
                        print(word_vectors.size())
                        word_vectors_np = word_vectors.cpu().numpy()[0]
                        print(word_vectors_np.shape)
                        # 计算相似性矩阵
                        similarity_matrix = np.dot(word_vectors_np, word_vectors_np.T)
                        ...
                        plt.xticks(np.arange(len(report)), report, rotation=90)
                        plt.yticks(np.arange(len(report)), report)        
                        # 添加颜色条并控制大小
                        colorbar = plt.colorbar(shrink=0.8)  # 通过调整shrink参数来控制颜色条的大小
                        # 保存为图片（PNG格式）
                        plt.savefig(os.path.join(self.save_dir, "attentions", "{:04d}".format(batch_idx),
                                                    "layer_{}".format(layer_idx),'similarity_matrix.png'), format='png')
                        plt.close() 

...
class ViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=1, num_classes=1000, embed_dim=768, depth=12,
    ...
    def forward(self, x):
        ...
        #for blk in self.blocks:
            x = blk(x)
        #for new_blk in self.new_blocks[:-1]:
            #x = new_blk(x)
        return x 
    ...
    def load_param(self, model_path):
        ...
        param_dict = {k.replace("base_encoder.", ""): v for k, v in param_dict.items()}
        param_dict = {k.replace("momentum_encoder.", ""): v for k, v in param_dict.items()}

        new_ckpt = OrderedDict()
    
        for k, v in param_dict.items():
            if 'atch_embed.proj.weight'in k:
                continue
            if 'mask_token'in k:
                continue
            if 'predictor'in k:
                continue
            if 'masked_embed'in k:
                continue
            if 'decoder'in k:
                continue
            if 'patch_embed.conv1.weight'in k:
...
def vit_large(img_size=(224, 224), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, **kwargs):
    model = ViT(

...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, num_mlp_heads=3, is_cam=False, 
                 is_extract_features=False, Qfomer=False, **kwargs):
        super().__init__()
        ...
        if self.is_Qformer:
            self.Qformer, self.disease_tokens = self.init_Qformer(14, self.num_features, False)
            hidden_size = self.Qformer.config.hidden_size
            self.qformer_linear = nn.Linear(hidden_size, self.num_features)
    ...
    def forward_features(self, x):
        ...
        # print(x.shape)
        if self.is_Qformer:
            disease_tokens = self.disease_tokens.expand(x.shape[0], -1, -1)
            image_atts = torch.ones(x.size()[:-1], dtype=torch.long).to('cuda')
            ...
            x = self.qformer_linear(query_output.last_hidden_state)
    ...
    def forward(self, x):
        ...
        if self.is_cam:
            output_tensor = torch.cat(y, dim=0)  # shape: (14, 2)
            pred_scores = F.softmax(output_tensor, dim=1)  # shape: (14, 2)
            positive_scores = pred_scores[:, 1].unsqueeze(0)  # 提取每个类别正类的概率
            return positive_scores
        elif self.is_extract_features:
            return x
        return y

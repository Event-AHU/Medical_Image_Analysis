...
def get_args():
    ...
    parser.add_argument(
        '--image-path',
        type=str,
        default='/wangx/_dataset/cheXpert_plus/PNG/train/patient42077/study1/view1_frontal.jpg',
...
if __name__ == '__main__':
    ...
    DISEASE_DICT ={}
    checkpoint_path = '/wangx/home/E23201049fa/SwinCheX/output/swin_large_patch4_window7_224/train_epoch100_1015/ckpt_epoch_25.pth'
    json_path = '/wangx/home/E23201049fa/SwinCheX/mimic_chexpert_label.json'
    # 读取json文件的内容
    ...
    # cam = AblationCAM(model=model, target_layers=target_layers, reshape_transform=reshape_transform) # not work
    # cam = ScoreCAM(model=model, target_layers=target_layers, reshape_transform=reshape_transform) # not work
    # cam = GradCAMPlusPlus(model=model, target_layers=target_layers, reshape_transform=reshape_transform) # work
    # cam = HiResCAM(model=model, target_layers=target_layers, reshape_transform=reshape_transform) # work
    # cam = EigenCAM(model=model, target_layers=target_layers, reshape_transform=reshape_transform) # 
    # cam = XGradCAM(model=model, target_layers=target_layers, reshape_transform=reshape_transform) # work
    # cam = FullGrad(model=model, target_layers=target_layers, reshape_transform=reshape_transform) # not work
    ...
    for data in tqdm(datas, total=len(datas)):
        ...
        for index, label in enumerate(labels):
            ...
            # cv2.imwrite('/rydata/wangfuling/IEEE_TMI/Visualization/norm.jpg', visualization)
            # from PIL import Image
            # Image.open('test.jpg')
            # breakpoint()
            # 定义 Patch 的大小
            patch_size = 16
            # 获取每个 Patch 的最大值或平均值，并与阈值比较
            patches = []
            ...
            for idx in selected_patch_indices:
                ...
                patch_output = linear(patch_output)  # 变为 (768)
    ...
    f_save = open('/rydata/wangfuling/IEEE_TMI/different_CAM/EigenCAM.pkl', 'wb')
